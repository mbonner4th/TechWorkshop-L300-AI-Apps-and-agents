...
def _normalize_endpoint_for_openai_v1(endpoint: str) -> str:
    """Normalize env endpoint to scheme+host for OpenAI v1 calls.
    ...
    if not parsed.scheme or not parsed.netloc:
        raise ValueError(
            "gpt_endpoint must be an absolute URL, e.g. "
            "https://<resource>.services.ai.azure.com"
        )
...
def get_chat_client() -> OpenAIChatClient:
    ...
    endpoint_root = _normalize_endpoint_for_openai_v1(endpoint)

    return OpenAIChatClient(
        model=deployment_name,
        base_url=f"{endpoint_root}/openai/v1/",
